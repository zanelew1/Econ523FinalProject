# ============================================================
# Replication Analysis Script (Fixed)
# ============================================================

# Load data
rm(list = ls())
require(install.load)
setwd("/Users/connorlewis_macbookpro/Documents/Personal Filing System/Permanent/Education Connor/2025AUG-2026MAY_MA Economics_UTK/01_Coursework/2026_Spring/Econ_523_Applied_Macroeconomic_Theory/Assignments/FinalProj/HSOHCP-Final-Submission/DATA/R")
require(readxl)
library(seasonal)
require(dynlm)
require(car)
...
# ============================================================
all_stata2 <- read_excel("all_consumption.xlsx")
all_stata2 = all_stata2[,-1]
varlist2 = names(all_stata2)

# Save each group varlist under a distinct name — do NOT overwrite
varlist2_NC      = varlist2[str_detect(varlist2, "NC_")]       # Nondurable consumption group
varlist2_DC      = varlist2[str_detect(varlist2, "DC_")]       # Durable consumption group
varlist2_SWITCHER = varlist2[str_detect(varlist2, "SWITCHER_")] # Switcher group
...
seasonalize_group(varlist2_NC,       all_stata2, use_outlier = TRUE)
seasonalize_group(varlist2_DC,       all_stata2, use_outlier = TRUE)
seasonalize_group(varlist2_SWITCHER, all_stata2, use_outlier = FALSE)  # switcher needs outlier suppressed
...
# ============================================================
# Second Stage: Loop over ALL group variables
# ============================================================
horizon2 = 9
lag = 4
...
# Combine all group variables into one list
all_groups = c(varlist2_NC, varlist2_DC, varlist2_SWITCHER)
